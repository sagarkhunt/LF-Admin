...
const AnnouncementList = () => {
  ...
  const {
    ...
    counts: store?.annoucement?.announcementListData,
    announcementListData: store?.annoucement?.announcementListData?.results,
    deleteAnnouncementData: store?.annoucement?.deleteAnnouncementData,
    updateStatusData: store?.annoucement?.updateStatus,
    status: store?.annoucement?.updateStatus,
  }));
  ...
  useEffect(() => {
    dispatch(announcementList({ page: page, limit: limit, search: search }));
  }, [page, limit, search, deleteAnnouncementData, updateStatusData]);
  ...
  useEffect(() => {
    if (announcementListData) {
      setAnnounceData([...announcementListData]);
    }
  }, [announcementListData]);
  ...
  const ColumnHeaders = () => (
    <>
      ...
      <th className="">Announcement Image</th>
            ...
            <div className="form-check form-switch">
              <Input
                ...
                onChange={(e) => {
                  ...
                  dispatch(updateStatus({ id: row?._id }));
          ...
          <td>
            <div className="d-flex justify-content-center align-items-center gap-1">
              ...
              <Edit
                ...
                onClick={() => {
                  history.push(
                    `announcement/add-announcement?announcementId=${row?._id}`
                  );
                }}
              />
              <UncontrolledTooltip placement="top" target={`edit-${row?._id}`}>
  ...
  return (
    <>
      ...
      <Row>
        <Col xs={6}>
          <BreadCrumbs
            breadCrumbTitle="Announce"
            breadCrumbActive="Announce List"
          />
        </Col>
        ...
        <Col xs={6} className="d-flex justify-content-end">
          <Button
            color="primary"
            onClick={() => history.push(`/announcement/add-announcement`)}
          >
            Add Announcements
          </Button>
        </Col>
