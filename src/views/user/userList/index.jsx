...
const UserList = ({ searchUser }) => {
  const dispatch = useDispatch();
  const history = useHistory();

  const { pageCounts, userListData } = useSelector((store) => ({
    pageCounts: store?.user?.userListData,
    userListData: store?.user?.userListData?.results,
  }));
  ...
  useEffect(() => {
    dispatch(userList({ page, limit, searchUser, userFilter: false }));
          ...
          <td>
            <div className="d-flex justify-content-center align-items-center gap-1">
              <Eye
                ...
                className="cursor-pointer"
                onClick={() =>
                  history.push(`/user/view-user?userId=${row?._id}`)
                }
              />
              <UncontrolledTooltip
                placement="top"
